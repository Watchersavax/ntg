...
import { CreatequestionDialogComponent } from '../add-template-dialog/designtemplate/createquestion-dialog/createquestion-dialog.component';
...
@Component({
  selector: "app-version-published-template-dialog",
  templateUrl: "./version-published-template-dialog.component.html",
  styleUrls: ["./version-published-template-dialog.component.css"]
})
export class VersionPublishedTemplateDialogComponent implements OnInit {

  newtemplateflag;
  rowelementdata :NewTemplateVersion ;
  templateid: number;
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,public dialogRef: MatDialogRef<VersionPublishedTemplateDialogComponent>,
    private route: ActivatedRoute,public dialog: MatDialog,private http:HttpClient,private templatedataservice:QuestionService,private paragraphservice:ParagraphConditionService
  ,private loadingsreenservice:LoadingscreenService,
    ...
    this.templateversion  = this.rowelementdata.templateVersionId;
  }
  ...
  ngOnInit() {
        ...
        blocks: [
          {
            ...
            label: "Text",
            content: `
              <span droppable=true  data-gjs-droppable="true" class="textstyle" > &nbsp;Insert your text here</span >
              <style>
                .textstyle{
                  font-style:inherit;
                  color:inherit;
    ...
    this.editor.on('canvas:drop', (event, component) => {
     ...
     component.set({removable: true});
      ...
      if(component.find('div > .row-cell') != undefined && component.find('div > .row-cell').length >0){
        for(let i = 0;i< component.find('div > .row-cell').length;i++){
          component.find('div > .row-cell')[i].set('attributes', { id: "identify"+this.universalelementid+1 });
        }
    ...
    this.editor.StorageManager.store({'html':this.rowelementdata.templateVersionValue,'css':this.rowelementdata.templateVersionCss,'components':this.rowelementdata.templateVersionComponent,'assets':this.rowelementdata.templateVersionAsset});
    this.editor.StorageManager.load(['html','css','components','assets']);
    this.editor.load(res => res);
  ...
  onEditorLoadEvent = (event) =>{
      ...
      this.universalelementid = this.editor.Canvas.getDocument().getElementsByClassName('row-cell').length+200;
 ...
 dynamicSort() {
  ...
  getQuestionGroups(){
    this.http.get(environment.url + "admin/template/getAllGroupTemplateQuestions/"+this.templateversion).subscribe( //+this.rowelementdata.publishedVersionId
      (data: any) => {
        ...
        for(let i=0;i<this.questionGroups.length;i++){
          for(let j=0;j<this.questionGroups[i]["templateQuestionDtos"].length;j++){
            this.questionlist.push(
              {
                "attributeId":this.questionGroups[i]["templateQuestionDtos"][j]["attributeDto"]["attributeId"],
        ...
        for(let i = 0 ;i<this.questionlist.length;i++){
          this.optioninnserhtmlstringforspan+='<option value="'+this.questionlist[i].attributeId+'"  style="color:white;background-color:#4b4d52;border:0px;outline:none">'+this.questionlist[i].questionText+'</option>' ;
        }
  ...
  addFonts = () => {
    ...
    styleManager.addProperty("typography",{name:"text-decoration",property:'text-decoration-line',type: 'select',
  ...
  setConditionTab() {
    const conditionElements = document.getElementsByClassName("condition");
    if (conditionElements.length > 0) {
      document.getElementsByClassName("condition")[0].id = "mycondi";
      document.getElementById("stylebutton").addEventListener("click", this.addClickListener);
      document.getElementById("settingsbutton").addEventListener("click", this.addClickListener);
      document.getElementById("layerbutton").addEventListener("click", this.addClickListener);
      document.getElementById("blocksbutton").addEventListener("click", this.addClickListener);
      document.getElementById("mycondi").addEventListener("click", this.addClickListener);
    }  
  }
  ...
  addingDropevent() {
    ...
    // row-cell
    if (document.getElementsByClassName("row-cell").length > 0) {
      for (
        let i = 0;
        i < document.getElementsByClassName("row").length;
        i++
      ) {this.universalelementid+=200;}
    }
  ...
  getListOfAttributes(){
    ...
    //need versionId
    this.http.get(environment.url+"admin/template/getAllTemplateAttributes/"+this.templateversion).subscribe((data:AttributeListResponse)=>{
      this.attributeDtoList = data.data;
      this.optioninnerhtmlstring = "";
      for(let i = 0 ;i<this.attributeDtoList.length;i++){
        this.optioninnerhtmlstring+='<option value="'+this.attributeDtoList[i].attributeId+'"  style="color:white;background-color:#4b4d52;border:0px;outline:none">'+this.attributeDtoList[i].attributeName+'</option>' ;
      }
  ...
  //this methods adds more conditions to condition tab for paragraph conditions 
  addMoreCondition = event => {
    ...
    let innerHtmlstring =
    '<button id="'+this.totalconditionforms +'" style="background-color: var(--custombtnColor);border: 0px;padding: 2px;font-size: x-small;font-weight: 500;width: 31%;height: 32px;border-radius: 3px;color: white;margin:5px;">Delete</button>'+
    ...
    document.getElementById(this.totalconditionforms+"").addEventListener('click',this.deleteCondition)
  };
  ...
  submitOfConditionForm = event => {
    ...
    requestObject["paragraphConditionId"] = this.universaleditconditionid;
    ...
    this.http.post(environment.url+"admin/template/saveParagraphCondition",requestObject).subscribe((data:ParagraphConditionResponse)=>{
      
      if(data.success === true){
      conditionresponseobject = data;
        newidforparagraph = "<#para"+conditionresponseobject.data.paragraphConditionId+"#>";
  ...
  selectConditionTabForSpan = event => {
    ...
    document.getElementById("conditionmanager").style.transition="0.7s ease all";
  ...
  ondropEvent(event) {
    ...
    if(this.idOfQuestionDragged!=null){
      ...
      let component:[] = this.editor.DomComponents.getWrapper().find('div .row-cell');
      if( component != undefined && component.length > 0 ){
        for(let i =0;i<component.length;i++){
          if(component[i]["attributes"]["attributes"]["id"] === event.target.id){
            ...
            this.universalspanid = this.universalspanid+200;
          }
        }
      }
...
checkQuestionAttributesAreAttached(group, question){
            ...
            for(let m=0;m<this.questionGroups[v]['templateQuestionDtos'][i]['questionConditionDtos'][j]['subconditionDtos'].length;m++){
              ...
              for(let k=0;k<quesitonAttributeList.length;k++){
                ...
                if(this.questionGroups[v]['templateQuestionDtos'][i]['questionConditionDtos'][j]['subconditionDtos'][m]['attribute']['attributeName'] == quesitonAttributeList[k]){
                  ...
                  message = 'Inside question visibility conditions of '+   this.questionGroups[v]['templateQuestionDtos'][i]['description'];
                  attachedflag = true;
  ...
  //check for those  conditions which are not anymore present in template paragraph conditions and remove them from paragraphconditions list
  let divelementarray:HTMLElement[] = _document.getElementsByTagName('div');
  let paraTemplateConditionList:number [] = [];

  for(let i=0;i<divelementarray.length;i++){
    if(divelementarray[i]["attributes"]["cust_tag"] != undefined){
      let attstring = divelementarray[i]["attributes"]["cust_tag"].value;
      
      let paraconditionId = parseInt(attstring.substring(attstring.indexOf('<#para')+6,attstring.indexOf('#>')));
      
      paraTemplateConditionList.push(paraconditionId);
    }
  }

  let tempParagraphConditionList:ParagraphSubconditionResponse [] = [];

  //remove those paragraph conditions from paragraphConditionsList which are not in use in the template 
  for(let i =0;i<this.paragraphConditionsList.length;i++){
    let findornot = false;

      for(let j=0;j<paraTemplateConditionList.length;j++){
        if(this.paragraphConditionsList[i].paragraphConditionId == paraTemplateConditionList[j]){
          findornot = true;
          break;
        }
      }
      ...
      if(findornot == true){
        tempParagraphConditionList.push(this.paragraphConditionsList[i]);
      }
  }

  this.paragraphConditionsList = [...tempParagraphConditionList];
  ...
  onQuestionDragEvent(event,group,originalevent) {
  ...
  saveTemplateData(event,closeflag){
    ...
    // //refresh canvas content in case some changes didnt reflected in storagemanager content

    let _document = this.editor.Canvas.getDocument();
    _document.getElementById('wrapper').click();
    this.wait(500);

      this.templaterequest = new NewTemplateVersion();
      this.templaterequest.versionValue=this.editor.getHtml();
      this.templaterequest.versionCss = this.editor.getCss();
      this.templaterequest.templateId = this.rowelementdata.templateId;
      this.templaterequest.versionId = this.rowelementdata.templateVersionId;
      this.templaterequest.archive = null;
      this.templaterequest.published = this.rowelementdata.published;
      this.templaterequest.versionName = this.rowelementdata.templateVersionName;
      this.templaterequest.versionComponent = localStorage.getItem('admin-components');
      this.templaterequest.versionAsset = localStorage.getItem('admin-assets');
      this.templaterequest.versionStyles = localStorage.getItem('admin-styles');
      this.templaterequest.templateVersionDescriptionHtml = this.rowelementdata.templateVersionDescriptionHtml;

    this.http.post(environment.url + "admin/template/updateTemplateDesign",this.templaterequest).subscribe(
      (data: any) => {

       if(data["success"] === true){
         if(closeflag === true){
          this.clearTemplateData(null);
          this.dialogRef.close(this.templaterequest);
         }else{
  ...
  close(){
    ...
    if(this.templaterequest === null){
      this.dialogRef.close(this.rowelementdata);
    }else{
    this.dialogRef.close(this.templaterequest);
    }
  }
