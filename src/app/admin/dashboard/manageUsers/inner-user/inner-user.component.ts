...
import { CreateGeneralUserComponent } from '../create-general-user/create-general-user.component';
import { DeleteUserDialogComponent } from '../manageUsers.dialog/delete-user-dialog/delete-user-dialog.component';
import { EditGeneralUserComponent } from '../edit-general-user/edit-general-user.component';
import { ActiveRole } from '../userModels/ActiveRole';
import { SelectorModel } from '../userModels/SelectorModel';
import { ToastrService } from 'ngx-toastr';

@Component({
  selector: 'app-inner-user',
  templateUrl: './inner-user.component.html',
  styleUrls: ['./inner-user.component.css']
})
...
export class InnerUserComponent implements OnInit {
  ...
  createNewUser() {
    ...
    const dialogRef = this.dialog.open(CreateGeneralUserComponent, {width:'35vw', data: this.activeRole});
  ...
  editUser(user: Users) {
    const dialogRef = this.dialog.open(EditGeneralUserComponent, {width: "35vw", data: { user: {...user}, activeRole: this.activeRole}});
  ...
  OnTemplateClick(user: Users) {
    this.router.navigate(['/admin', 'dashboard', 'users', 'affidavit'],
  ...
  deleteUser(user: Users){
    this.dialog.open(DeleteUserDialogComponent, 
      { data: {  message: "Deleting this user will result in a loss of all data associated with them. Are you sure you want to delete this user’s profile?", }})
        .afterClosed()
        .subscribe(data => {
          if (data === 'Yes') {
